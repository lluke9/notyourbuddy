...
def chat() -> object:
    ...
    client_last_word = (payload or {}).get("lastBotWord", "buddy")
    if not isinstance(client_last_word, str):
        client_last_word = "buddy"
    client_last_word = client_last_word.strip() or "buddy"

    if not isinstance(message, str):
        state = get_state()
        return jsonify({
            "reply": f"What's that, {client_last_word}?",
            "score": state.get("score", 0),
            "lastBotWord": client_last_word,
        })
    ...
    if lowered == "::reset":
        reset_state()
        return jsonify({
            "reply": "Fresh start. Hit me again.",
            "score": 0,
            "command": True,
            "lastBotWord": "buddy",
        })
    ...
    if user_norm in state["used"] or user_norm in state["bot_used"]:
        ...
        if reply_word:
            ...
            last_word = reply_word
        else:
            response = f"You ruined it, but I'm out of comebacks. Score: {score}."
            last_word = client_last_word
        return jsonify({"reply": response, "score": score, "lastBotWord": last_word})
    ...
    reply_word = pick_reply_word(state, disallow=response_norm)
    if not reply_word:
        reply_word = client_last_word or "buddy"
    response = f"I'm not your {canonical_terms[-1]}, {reply_word}."
    return jsonify({"reply": response, "score": state["score"], "lastBotWord": reply_word})
